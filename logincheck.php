...
$query=mysqli_query($dbcon,"select * from userlogin where pnid='$username' AND password= SHA1('$pwd')");

$count=mysqli_num_rows($query);
if($count ==1){
	$row = mysqli_fetch_array($query);
$_SESSION['pnid']=$row['pnid'];
...
//mysqli_query($dbcon,"UPDATE voter SET vote_status='voted' where index_number='$voterid'");
	...
	$_SESSION['error'] = 'Your NID or password is not valid';
	}
